  ...
  export default {
    ...
    methods: {
      ...
      submitFiles() {
        ...
        this.$axios.$post('/webapi/Image/PostUploadImage?imageTypeId=6&id=' + this.categoryId,
        formData,
        {
            headers: {
            ...
            _this.files = []
            _this.updateMenuImage(response)
        })
        .catch(function(error){
            console.log(error);
        })
        },
        updateMenuImage(response) {
            this.$emit('updateMenuImage', response)
        },
