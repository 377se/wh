<template>
  <div>
    ...
    <div class="uk-grid-column-medium" uk-grid>
      <div>
        <button class="uk-button" v-on:click="addFiles()">VÄLJ BILD</button>
      </div>
      <div >
  ...
  export default {
    ...
    methods: {
      ...
      submitFiles() {
        ...
        this.$axios.$post('/webapi/Image/PostUploadImage?imageTypeId=9&id=' + this.bannerId,
        formData,
        {
            headers: {
            ...
            _this.files = []
            _this.updateBannerImage(response)
        })
        .catch(function(error){
            console.log(error);
        })
        },
        updateBannerImage(response) {
            this.$emit('updateBannerImage', response)
        },
