<template>
    ...
    <div v-else>
        <div id="sc-page-wrapper">
            ...
            <div id="sc-page-content" v-if="orderList">
                <ScCard :key="render">
                    <ScCardBody>
        ...
        <div style="visibility: hidden;" v-if="orders">
...
import Deliverynotes from '~/components/Deliverynotes-test'
...
export default {
    ...
    methods: {
        ...
        async getOrderList() {
            ...
            _this.orders = []
            _this.$store.dispatch('setBusyOn')
			await this.$axios.$get('/webapi/Test/GetPrintoutTest')
			.then(function (orderlist) {
                if (orderlist.ActivationError.OrderId != 0 ) {
                    _this.activationError = orderlist.ActivationError
                    _this.$store.dispatch('setBusyOff')
                    ...
                    _this.orderList = orderlist.OrderList
                    _this.resetIsSelected()
                    _this.$store.dispatch('setBusyOff')
                }
			})
    ...
    async fetch () {
        ...
        try {
            const [ shops, testorders ] = await Promise.all([
                this.$axios.$get('/webapi/Shop/GetShopList'),
                this.$axios.$get('/webapi/Test/GetPrintoutTest'),
            ])
            this.shopOptionsList = shops.map(({ ShopId, ShopName }) => ({ id: ShopId, text: ShopName }))
            this.orderList = testorders.OrderList
